...
const MusicNote = ({ size = 16, color = 'currentColor', className = '' }) => (
  <svg
    width={size}
    height={size}
    viewBox="0 0 24 24"
...
// Public-domain treble clef (Wikimedia Commons), scaled to a 1500x4260 viewBox
...
const TrebleClef = ({ size = 160, color = 'currentColor', className = '', style }) => (
  <svg
    width={size}
    height={Math.round(size * 2.84)}
    viewBox="1140 5460 1500 4260"
    fill={color}
    className={className}
    style={style}
    aria-hidden="true"
...
const Vinyl = ({ size = 200, color = 'currentColor', className = '' }) => (
  <svg
    width={size}
    height={size}
    viewBox="0 0 100 100"
  ...
  >
    <g stroke={color} fill="none">
      ...
      <circle cx="50" cy="50" r="39" strokeWidth="0.6" opacity="0.7" />
      <circle cx="50" cy="50" r="30" strokeWidth="0.6" opacity="0.5" />
      <circle cx="50" cy="50" r="21" strokeWidth="0.8" opacity="0.6" />
      <circle cx="50" cy="50" r="12" strokeWidth="0.8" opacity="0.7" />
    </g>
    <circle cx="50" cy="50" r="3.2" fill={color} />
  </svg>
)

const Equalizer = ({ bars = 5, size = 16, color = 'var(--cui-primary)', className = '' }) => (
  <div
    className={`equalizer ${className}`.trim()}
    style={{ '--eq-size': `${size}px`, '--eq-color': color }}
    role="img"
    aria-label="Música en vivo"
  >
...
const StaffDivider = ({ caption, className = '', subtle = false }) => (
  <div className={`staff-divider ${className}`.trim()} role="presentation" aria-hidden="true">
    <div className="staff-divider-inner">
      <span className="staff-divider-lines" />
      {!subtle && (
        <span className="staff-divider-notes">
          <MusicNote size={13} className="staff-note staff-note--lead" />
          <MusicNote size={13} className="staff-note" />
        </span>
      )}
      <span className="staff-divider-lines" />
    </div>
    {caption && <p className="staff-caption mb-0">{caption}</p>}
  </div>
)

export { MusicNote, TrebleClef, Vinyl, Equalizer, StaffDivider }
export default StaffDivider
